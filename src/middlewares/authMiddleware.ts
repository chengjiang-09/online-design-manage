...
async function authMiddleware(ctx: Context, next: Next) {
  ...
  //校验jwt白名单
  if (
    !config.jwt.jwt_unless.includes(path) &&
    !config.jwt.jwt_unless.includes(`${path} ${ctx.method}`)
  ) {
    ...
    if (token !== undefined && token !== "") {
      ...
      const verifyResult = await redisGet(
        `${config.redis.redis_verify_token.key}${userData?.payload?.email}`
      );
